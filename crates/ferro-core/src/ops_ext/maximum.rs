...
//! Unlike `f32::max`, NaN in either operand propagates to the output rather
//! than being silently discarded. Backward: gradient flows to whichever
//! operand is larger; at a tie (a == b) it splits 0.5/0.5, matching torch.
//! da = g * (a>=b ? (a>b?1:0.5) : 0), db = g * (b>=a ? (b>a?1:0.5) : 0),
//! each unbroadcast to its input's shape; NaN in either operand propagates
//! to both partials too.
...
impl Tensor {
    pub fn maximum(&self, other: &Tensor) -> Result<Tensor> {
        let out = raw_binary("maximum", self, other, |a, b| {
