...
fn cumsum_values_2d_both_dims() {
    ...
    assert_eq!(a.cumsum(0).unwrap().to_vec(), vec![1.0, 2.0, 3.0, 5.0, 7.0, 9.0]);
    assert_eq!(a.cumsum(1).unwrap().to_vec(), vec![1.0, 3.0, 6.0, 4.0, 9.0, 15.0]);
}
...
fn cumsum_grad() {
    ...
    grad_check(&[a.clone()], |t| t[0].cumsum(1).unwrap().mul(&t[0]).unwrap().sum());
