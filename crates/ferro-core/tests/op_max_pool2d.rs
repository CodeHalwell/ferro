...
fn max_pool_values() {
    ...
    let b = Tensor::from_vec(
        vec![1.0, 5.0, 2.0, 3.0, 4.0, 6.0, 9.0, 8.0, 7.0],
        &[1, 1, 3, 3],
    )
    .unwrap();
...
fn max_pool_grad() {
    ...
    let vals = vec![
        0.3, 1.2, -0.5, 2.1, 0.9, -1.3, 1.6, 0.1, -0.8, 2.4, 0.6, -0.2, 1.9, 0.4, -1.1, 0.8,
    ];

    let a = Tensor::from_vec(vals.clone(), &[1, 1, 4, 4]).unwrap();
    let w = Tensor::from_vec(vec![0.5, -1.0, 2.0, 3.0], &[1, 1, 2, 2]).unwrap();
    grad_check(&[a], move |t| {
        t[0].max_pool2d(2, 2).unwrap().mul(&w).unwrap().sum()
    });
    ...
    let w2 = Tensor::from_vec(
        vec![0.5, -1.0, 2.0, 3.0, -0.5, 1.5, 2.5, -2.0, 1.0],
        &[1, 1, 3, 3],
    )
    .unwrap();
    grad_check(&[b], move |t| {
        t[0].max_pool2d(2, 1).unwrap().mul(&w2).unwrap().sum()
    });
}
