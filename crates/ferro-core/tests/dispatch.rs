...
fn unregistered_device_errors() {
    ...
    assert!(
        msg.contains("no backend registered for device cuda:7"),
        "got: {msg}"
    );
}
...
fn registry_routes_per_device() {
    ...
    assert_eq!(
        fake.unary(UnaryKind::Neg, &[1.0, 2.0]),
        vec![1001.0, 1002.0]
    );
...
fn cpu_binary_kinds_match_reference() {
    ...
    assert_eq!(
        CpuBackend.binary(BinaryKind::Add, &a, &b),
        zip().map(|(&x, &y)| x + y).collect::<Vec<_>>()
    );
    assert_eq!(
        CpuBackend.binary(BinaryKind::Sub, &a, &b),
        zip().map(|(&x, &y)| x - y).collect::<Vec<_>>()
    );
    assert_eq!(
        CpuBackend.binary(BinaryKind::Mul, &a, &b),
        zip().map(|(&x, &y)| x * y).collect::<Vec<_>>()
    );
    assert_eq!(
        CpuBackend.binary(BinaryKind::Div, &a, &b),
        zip().map(|(&x, &y)| x / y).collect::<Vec<_>>()
    );
}
