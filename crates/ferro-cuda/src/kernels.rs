...
/// Materialize a strided view in row-major order, one thread per output element.
/// Decode output coordinates and read x[offset + sum(coord[d] * stride[d])].
/// Rank (at most 16), metadata arithmetic and source bounds are checked on the host.
...
/// bounds-checked on the host before launch, so no index guard is needed.
