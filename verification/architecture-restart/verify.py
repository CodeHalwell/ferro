"""Build and exercise the CPU architecture gates in a fresh evidence directory."""
import argparse
import hashlib
...
def sources():
    paths = subprocess.check_output(['git', 'ls-files', '--cached', '--others', '--exclude-standard',
                                     'crates', 'examples', 'verification/architecture-restart/verify.py', '.github/workflows/correctness.yml'], cwd=ROOT, text=True)
...
def main():
    ...
    env.pop('FERRO_REQUIRE_CUDA', None)
    if os.name == 'nt':
    ...
    provenance = {'head': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
                  ...
                  'package_hashes': wheel_hashes, 'proof_device': 'cpu', 'cuda_required': False}
    ...
    print('All CPU gates passed; source and installed identities unchanged.', flush=True)
